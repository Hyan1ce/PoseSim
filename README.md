...
本项目使用互联网上的动作视频数据进行测试。视频格式为 MP4，分辨率范围 720p-1080p，帧率 25-30 fps。示例数据已置于 `input/` 目录供参考使用。

## 实验结果分析

### 检测性能

在测试环境（AMD EPYC 7H12 64-Core Processor 处理器）下，满足准实时处理需求。模型复杂度参数对处理速度影响显著，复杂度为0时速度最快但精度略降，复杂度为2时精度最高但速度较慢。

### 检测精度

从`output`中的示例结果视频可以看出， 动作幅度较大时检测效果显著；动作幅度较小时会出现一些检测误差，如错误检测关节位置，或检测精度下降。不过大多数动作都能够进行精准识别并绘制。
